...
export const EnhancedHomeScreen: React.FC<EnhancedHomeScreenProps> = ({ navigation }) => {
  ...
  const renderRecommendedIslands = () => {
    if (isSmartIslandEnabled) {
      // Use smart island selector when feature flag is enabled
      return (
        <View style={styles.section}>
          <SmartIslandSelector
            onIslandSelect={(island) => {
              actions.logNavigationEvent('smart_island_selected', { island });
              navigation.navigate(ROUTES.SEARCH, { island });
            }}
            showDistances={true}
    ...
    // Fallback to basic island selection when smart selection is disabled
    return (
      <View style={styles.section}>
