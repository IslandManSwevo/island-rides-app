...
- **Verification:** driver's-license photo + selfie captured at first booking (or from Profile). Provider (e.g. Stripe Identity) deferred; the flow and `verificationStatus` field are designed now.
...
flowchart TD
    ...
    G --> I[Earnings: payout ~3 days after trip start<br/>via Stripe Connect]
```
